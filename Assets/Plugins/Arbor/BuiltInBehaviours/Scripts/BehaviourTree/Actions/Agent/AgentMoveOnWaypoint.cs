...
namespace Arbor.BehaviourTree.Actions
{
	...
	public sealed class AgentMoveOnWaypoint : AgentUpdateBase
	{
		...
		private bool _IsStartExecute = false;
		...
		{
			if (!_IsStartExecute || agentController.isDone)
			{
				if (!GotoNextPoint(agentController, _IsStartExecute))
				{
					return false;
				}
			}
			_IsStartExecute = true;
