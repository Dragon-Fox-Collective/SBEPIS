...
namespace VoxelSystem {
    ...
    public class CPUVoxelizer {
        ...
        public static void Voxelize (Mesh mesh, int resolution, out List<Voxel_t> voxels, out float unit, out float[,,] voxelField, out Vector3 start) {
            ...
            voxelField = new float[width, height, depth];
            for(int x = 0; x < width; x++) {
                for(int y = 0; y < height; y++) {
                    for(int z = 0; z < depth; z++) {
                        ...
                        voxelField[x, y, z] = volume[x, y, z].IsEmpty() ? 1f : 0f;
                    }
                }
            }
        }
