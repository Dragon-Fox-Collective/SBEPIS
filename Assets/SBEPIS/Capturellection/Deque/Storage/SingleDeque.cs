...
namespace SBEPIS.Capturellection.Storage
{
	public abstract class SingleDeque<TState> : MonoBehaviour, DequeRuleset<TState> where TState : InventoryState, DirectionState, new()
	{
		...
		public virtual UniTask<StoreResult> StoreItem(TState state, Capturellectable item) => state.Inventory[Mathf.Max(state.Inventory.FindIndex(storable => !storable.HasAllCardsFull), 0)].StoreItem(item);
