...
namespace SBEPIS.Capturellection.Storage
{
	public class DequeRulesetLayer : DequeRuleset<DequeRulesetLayerState>
	{
		...
		public override UniTask							FlushCardPostHook	(List<Storable> inventory, DequeRulesetLayerState layerState, Storable storable)									=> Zip(			layerState).ForEach((ruleset, state)								=> ruleset.FlushCardPostHook(inventory, state, storable));
		...
		public override IEnumerable<Storable>			LoadCardPreHook		(List<Storable> inventory, DequeRulesetLayerState layerState, Storable storable)									=> Aggregate(	layerState, ExtensionMethods.EnumerableOf(storable), (result, ruleset, state) => result.SelectMany(res => ruleset.LoadCardPreHook(inventory, state, res)).Where(r => r));
		public override void							LoadCardPostHook	(List<Storable> inventory, DequeRulesetLayerState layerState, Storable storable)									=> Zip(			layerState).ForEach((ruleset, state)								=> ruleset.LoadCardPostHook	(inventory, state, storable));
		public override InventoryStorable				SaveCardHook		(List<Storable> inventory, DequeRulesetLayerState layerState, InventoryStorable oldCard)							=> Aggregate(	layerState, oldCard, (result, ruleset, state)	=> ruleset.SaveCardHook		(inventory, state, result));
