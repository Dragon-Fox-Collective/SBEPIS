...
{
	public class ItemGrabberFlatscreen : ItemGrabber
	{
		...
		private void FixedUpdate()
		{
			...
			heldGrabbable.OnHeld(this);

			if (RaycastPlacementHelper(out PlacementHelper placement, heldGrabbable.itemkind))
				UpdateItemSnapToPlacementHelper(heldGrabbable, placement);
			else
				UpdateItem(heldGrabbable, true, 0.05f, 0.05f);
		}
		...
		{
			if (!context.performed || !heldGrabbable)
				return;

			Quaternion lookRot = Quaternion.LookRotation(activeHolder.position - heldGrabbable.transform.position, activeHolder.up);
			Quaternion upRot = lookRot * Quaternion.Euler(0, 180, 0); // Front facing player
			Quaternion downRot = lookRot; // Back facing player

			if (cardForcedRotTarget == Quaternion.identity)
				cardForcedRotTarget = Quaternion.Angle(heldGrabbable.transform.rotation, upRot) > 90 ? upRot : downRot;
			else
		...
		public void OnPickUp(CallbackContext context)
		{
			...
			if (isPressed && !heldGrabbable && Cast(out RaycastHit hit, LayerMask.GetMask("Button")))
				hit.rigidbody.GetComponent<PhysicsButton>().ForcePress();

			if (isPressed && !heldGrabbable && Cast(out hit))
			{
				Item hitItem = hit.rigidbody.GetComponent<Item>();
				if (hitItem && hitItem.canPickUp)
					(heldGrabbable = hitItem).OnPickedUp(this);
			}
		...
		{
			Item droppedItem = heldGrabbable;
			heldGrabbable = null;
			cardForcedRotTarget = Quaternion.identity;
