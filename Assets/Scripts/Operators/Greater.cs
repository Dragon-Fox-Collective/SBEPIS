...
public class Greater : Operator
{
	protected override CaptchalogueCard Operate(CaptchalogueCard card1, CaptchalogueCard card2)
	{
		...
		string captcha1 = ItemType.unhashCaptcha(card1.itemHash);
		string captcha2 = ItemType.unhashCaptcha(card2.itemHash);
		string captcha3 = "";
		for (int i = 0; i < 8; i++)
		...
		newCard.itemHash = ItemType.hashCaptcha(captcha3);
		return newCard;
	}
}
